...
const validateSearchRequest = (data) => {
    const schema = joi_1.default.object({
        ...
        country: joi_1.default.string().required().min(2).max(2).uppercase(),
        city: joi_1.default.string().optional().allow('').max(100).trim(),
        state: joi_1.default.string().optional().allow('').max(50).trim(),
        postalCode: joi_1.default.string().optional().allow('').max(20).trim(),
        language: joi_1.default.string().optional().min(2).max(2).lowercase().default('en'),
