...
export const validateApiKey = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!apiKey) {
    logger.warn(`API key required but not provided from IP: ${req.ip}`);
    res.status(401).json({
      success: false,
      message: 'API key required. Please provide an API key in the x-api-key header or request body.',
